...
plt.plot(x, y, 'D-k', label='x cubed') # make the line between diamonds solid
plt.plot(x, y2, '--r', label='x squared') # add another line
plt.title('As x increases, \nx Cubed (black) increases \nat a Greater Rate than \nx Squared (red)', fontsize=22) # increase title font size
...
plt.xlabel('Linearly Spaced Numbers') # add x axis label
plt.ylabel('y Value')
plt.title('x by x Cubed', fontsize=22) # increase title font size
plt.legend(loc='upper left') # create a plot legend and place it in the upper left
plt.show() # print the plot
###############################################################################
